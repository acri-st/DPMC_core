...
# 2018-11-13 :  gb : state_vector set to 0 or 1 as a function of the product name
#                    for dynamic products (S3A*=0, S3B*=1, other=0)
#                    version set to satellite acronym (S3A/S3B/S3_) for
#                    auxiliary products
# 2019-01-22 :  gb : state_vector set to 9 if not S3A or S3B
#
# -------------------------------------------------------------------
...
RANDOM_FOLDER="${SYSTEM_TMP}/${RANDOM}"
MANIFEST_FILE="${RANDOM_FOLDER}/${PRD_NAME}/${PRD_NAME}/xfdumanifest.xml"

# create a working folder (to be removed at the end of processing)
if [ -f ${PRD_PATH} ]; then
  mkdir -p ${RANDOM_FOLDER}/${PRD_NAME}
  unzip -d ${RANDOM_FOLDER}/${PRD_NAME} ${PRD_PATH} ${PRD_NAME}/xfdumanifest.xml >> /dev/null
  PRD_SIZE=$(stat --format "%s" $PRD_PATH)
else
  mkdir -p ${RANDOM_FOLDER}/${PRD_NAME}/${PRD_NAME}
  cp ${PRD_PATH}/xfdumanifest.xml ${RANDOM_FOLDER}/${PRD_NAME}/${PRD_NAME}/xfdumanifest.xml
...
if [ ! -f ${MANIFEST_FILE} ]; then
  ...
  /bin/rm -rf ${RANDOM_FOLDER}/${PRD_NAME}
  exit 7
fi
...
# clean ${RANDOM_FOLDER} folder
/bin/rm -rf ${RANDOM_FOLDER}
...
if [ $MODE = "S" ]; then
  ...
  if [ $AUX -eq 0 ]; then
    ...
    # 2018/11/13 : gb : set different state_vector (i.e. satellite ids) as a function of the product name
    if [ "${SATELLITE_ACRONYM}" = "S3A" ]; then
      PRD_STATE_VECTOR_ID=0
    elif [ "${SATELLITE_ACRONYM}" = "S3B" ]; then
      PRD_STATE_VECTOR_ID=1
    else
      PRD_STATE_VECTOR_ID=9
    fi
  ...
  if [ $AUX -eq 0 ]; then
    ...
    # 2018/11/13 : gb : get satellite name 
    SATELLITE_ACRONYM=$(echo ${PRD_NAME} | cut -c1-3)
    echo "Satellite acronym used as product version : ${SATELLITE_ACRONYM}"

    # auxiliary product... add internal.auxiliary_product record (note: version field is 
    # set to the satellite acronym S3A, S3B or S3_)
    $(${PSQL_CMD} "INSERT INTO internal.auxiliary_product (product, validity_start_date_time,
      validity_stop_date_time, version)
      VALUES(${PRD_ID}, timestamp '${PRD_START_TIME}', timestamp '${PRD_STOP_TIME}', '${SATELLITE_ACRONYM}');")
    ERR=$?
...
if [ $AUX -eq 0 ]; then
  ...
  TMP_LOG=$(mktemp)
  # Run PSQL function to create disk location (media_catalog and media_catalog_entry records
  ${PSQL_CMD} "SELECT * FROM internal.disk_location_create('${PRD_NAME}', '${PRD_PATH}');" 2> $TMP_LOG
  ERR=$?

  # Store temporary log file content
  ERR_LOG=$(cat $TMP_LOG)
  # Delete temporary log file
  rm -f $TMP_LOG

  # Check if the error contains 'is a valid disk_location format'
  RES=$(echo $ERR_LOG | grep "is a valid disk_location format")

  # If the check result is not empty, it means it is a valid disk lcoation format error, so do nothing
  if [ "$RES" != "" ]; then
    echo "The path $PRD_PATH has already been created"
  # For other errors exit with error code
  else
    if [ $ERR -ne 0 ]; then
      echo "insert_product: product $PRD_NAME not referenced. Error $ERR during disk_location_create!"

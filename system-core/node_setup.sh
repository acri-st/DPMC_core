...
# -------------------------------------------------------------------
#
# This script is use to change the node characteristics
#
# History:
#
# 2018-02-23 �  gb : initial version
#
# -------------------------------------------------------------------
...
print_syntax() {
  echo
  echo " Syntax: $0 -c nb_cores -n node_id|-a "
  echo
  echo " -c + number of jobs "
  echo " -n + node identifier "
  echo " -a (all nodes between 70 and 99) "
  echo
}
...
if [ $NJOBS -eq 999 ]; then
  print_syntax
  echo
  echo " -----> Error: number of jobs shall be specified !"
  echo
  exit 1
fi
...
if ! $ALL ; then
  if [ $NODE -eq 999 ]; then
    print_syntax
    echo
    echo " -----> Error: node Id shall be specified or (-a) for all nodes !"
    echo
    exit 1
  fi
fi
...
if $ALL ; then
  for NODE in $(seq 70 99) ; do
    #${PSQL_CMD} "UPDATE processing.hosts SET ncpu=$NJOBS WHERE host_id=$NODE;"
    echo $NODE $NJOBS
  done
elif [ $NODE -ge 70 -a $NODE -le 99 ]; then
  #${PSQL_CMD} "UPDATE processing.hosts SET ncpu=$NJOBS WHERE host_id=$NODE;"
  echo $NODE $NJOBS
fi

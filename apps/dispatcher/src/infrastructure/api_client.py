...
async def expand_task_via_api(api_url: str, api_token: str, task_id: int, *, ssl_verify: bool = True) -> None:
    """POST /scheduler/task/:id/expand. Raises on non-2xx."""
    headers = {"X-Worker-Token": api_token} if api_token else {}
    async with httpx.AsyncClient(base_url=api_url.rstrip("/"), timeout=60.0, verify=ssl_verify) as client:
        resp = await client.post(f"/scheduler/task/{task_id}/expand", headers=headers)
        resp.raise_for_status()


async def wait_for_api_ready(api_url: str, *, poll_interval_s: float = 2.0, ssl_verify: bool = True) -> None:
    ...
    while True:
        attempt += 1
        try:
            async with httpx.AsyncClient(timeout=2.0, verify=ssl_verify) as client:
